...
void fmt::Bcopy(Bigint *x, Bigint *y)
{
	memcpy(&x->sign, &y->sign, y->wds*sizeof(unsigned int) + 2 * sizeof(int));
}

void fmt::Bfree(Bigint *v)
{
	free((void*)v);
}

int fmt::lo0bits(unsigned int *y)
{
	int k;
	unsigned int x = *y;
...
Bigint* fmt::multadd(Bigint *b, int m, int a)
{
	int i, wds;
	unsigned int *x;
...
int fmt::hi0bits(unsigned int x)
{
	int k = 0;
...
Bigint* fmt::mult(Bigint *a, Bigint *b)
{
	Bigint *c;
	int k, wa, wb, wc;
...
Bigint* fmt::pow5mult(Bigint *b, int k)
{
	Bigint *b1, *p5, *p51;
	int i;
...
Bigint* fmt::lshift(Bigint *b, int k)
{
	int i, k1, n, n1;
	Bigint *b1;
...
Bigint* fmt::diff(Bigint *a, Bigint *b)
{
	Bigint *c;
	int i, wa, wb;
...
Bigint* fmt::d2b(double dd, int *e, int *bits)
{
	Bigint *b;
	int i;
	int de, k;
...
int fmt::quorem(Bigint *b, Bigint *S)
{
	int n;
